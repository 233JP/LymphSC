...
def get_atom_features(atom, d_atom):
    # 计算Gasteiger电荷
    if '_GasteigerCharge' not in atom.GetPropsAsDict():
        raise KeyError("Gasteiger charge not computed.")
...
if __name__ == '__main__':
    ...
    mol = Chem.MolFromSmiles('CCCCCCC(C)(C)C1=CC(O)=C2[C@H]3CC(CO)=CC[C@@H]3C(C)(C)OC2=C1')  # 示例分子乙醇
    compute_gasteiger_charges(mol)
    # for atom in mol.GetAtoms():
    features = get_atom_features(atom, mol)
    print(f'Atom: {atom.GetSymbol()} | Features: {features}')

    # 原始SMILES字符串
    # smiles = 'CCCCCCC(C)(C)C1=CC(O)=C2[C@H]3CC(CO)=CC[C@@H]3C(C)(C)OC2=C1'
